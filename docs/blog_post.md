# Securing CI/CD Pipelines with GitHub OIDC and AWS IAM: A Practical Guide
...
> **Note:**  
> By default, only the current repository will be granted access. To allow additional GitHub repositories to assume the same IAM role, add their names (in `org/repo` format, one per line) to the `allowed_repos.txt` file before running the automation. The script will generate the correct trust policy for all repos listed. You do **not** need to edit this file for single-repo setups.  
