...
### 4. Integrate into Your GitHub Actions Workflow

To use the IAM Role generated by the script, you can reference the IAM Role ARN directly in your workflow. This approach is suitable for solo use or quick setup, but can be adopted by any team that prefers explicit configuration.
...
Replace the ARN above with the value output by the script. You may also choose to use a repository variable for the IAM Role ARN if you prefer a more maintainable or team-friendly approach. Both options are supported by the script output.
